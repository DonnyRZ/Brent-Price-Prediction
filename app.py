...
def main():
    ...
    # Dashboard Sinyal Harian Brent (Utama)
    section_title("Dashboard Sinyal Harian Brent (Utama)")
    X_full_valid = X_full.dropna()
    latest_features = X_full_valid.iloc[[-1]]
    latest_close = data_full.loc[latest_features.index, "close_x"].iloc[-1]
    latest_pred = float(
        predict_all(model_key, model_artifacts[model_key], latest_features)[0]
    )
    delta = latest_pred - latest_close
    signal = "BUY" if latest_pred > latest_close else "SELL"

    kpi1, kpi2, kpi3, kpi4 = st.columns(4)
    kpi1.metric("Close Hari Ini", f"{latest_close:,.2f}")
    kpi2.metric("Prediksi Besok", f"{latest_pred:,.2f}")
    ...
    st.caption("Catatan: Prediksi merepresentasikan harga penutupan **hari berikutnya**.")
